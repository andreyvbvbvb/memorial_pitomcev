...
### `/map`
- Карта Google Maps + справа список мемориалов.
- Должны отображаться только мемориалы в текущих границах карты (это нужно починить).
- Кластеризация маркеров включена.
...
- «От:» — должно показывать «От хозяина» + список питомцев владельца (кликабельные).
- Наведение на подарок должно подсвечивать объект в 3D и показывать tooltip.

### `/my-pets`
- Список мемориалов пользователя, без лишней информации.
...
1. **/map**: список справа должен фильтроваться по видимой области карты. Сейчас фильтр есть, но список остаётся полным — нужно исправить.
2. **/map**: карта «самосмещается» после перемещения — вероятно из-за `fitBounds` или автозума/центровки.
3. **/create**: ошибки сборки, если отсутствует тип JSX `<group>` (three) — нужно добавить в `react-three-fiber.d.ts`.
4. **/pets/[id]**: увеличить высоту 3D превью на 20% (нужно проверить, как сделано на /create).
5. **/pets/[id]**: подсветка подарка при hover внутри 3D превью + tooltip в 3D.
6. **/pets/[id]**: улучшить вид и прозрачность маркеров (меньше и круглее).

## 10) Локальные фикс-паттерны (частые решения)

- **MapClient.tsx**:
  - Должно быть `center`/`zoom`, не `defaultCenter`/`defaultZoom`, иначе TS ошибка.
  - Фильтровать список по `visibleMarkers` (которые определяются через `map.getBounds()` на `onIdle`).
...
- **Map**: кластеризация + marker filtering по bounds — нужно завершить.
