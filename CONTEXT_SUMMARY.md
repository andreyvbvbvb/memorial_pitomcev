...
- **Header**:
  ...
  - в меню‑«книжке» — поп‑ап пополнения баланса (RUB/USD, выбор тарифа, кнопка “Продолжить” → `/payment`).
- **/map**:
...
- **Размер подарков**: для `star` выбранный размер (`s/m/l`) хранится в `GiftPlacement.size` и масштабируется при рендере.
