...
- 2‑шаговый мастер создания мемориала.
...
- **/map**:
  ...
  - Карточки мемориалов используют **скрин 3D‑превью** (отдельно от галереи/обложки маркера).
...
- **/pets/[id]** (мемориал):
  - Верх страницы: **имя**, **даты жизни**, линия и **эпитафия** по центру.
  - Фон страницы — мягкий голубоватый (`#eef6ff`).
  - Далее: галерея (включая обложку) → история.
  - Блок дарения (подарок/срок/слот) над 3D‑превью; **срок без дефолта**, кнопка активна только при полном выборе.
  - При нехватке средств открывается модал пополнения (как в хедере).
  ...
  - После 3D‑превью — “Другие мемориалы хозяина” (если их > 1), затем список подаренных.
...
- **Размер подарков**:
  ...
  - `candle_*` ширина ~0.39м (1–12 одинаково).
  - `flower_*` ширина ~0.6м.
  - `candle_9..12` цена **50 монет** (остальные свечи — 30).
- **Грязь**: одна GLB‑модель на домик `/models/dirt/{baseId}/dirt.glb` с сегментами `dirt_1..dirt_4`.
...
- **2 шага**.
- **Шаг 1**: центрированный блок основных данных (имя, вид, даты); кнопка “Продолжить” по центру.
- Между шагами — **полноэкранный экран загрузки**.
- **Шаг 2**: 3D‑превью на весь экран (фон), UI поверх:
  - справа — редактор деталей с вертикальными вкладками (иконки слева);
  - слева — кнопки‑иконки окон: основные данные, история, маркер, фото;
  - “Завершить” открывает модал проверки и оплаты (баланс подгружается каждый раз).
- **Маркер**: расширенное окно (≈2× шире), карта слева, справа — выбор маркера; виды животных — вертикальные иконки с tooltip; есть ручной ввод координат.
- **Фото**: максимум 10 файлов, до 6 МБ; выбор обложки для маркера.
- **Время года**: квадраты‑свотчи (лето/осень/весна/зима) + авто‑смена (по умолчанию выключена, подсказка по hover).
- **Превью подарков**: чекбокс “Посмотреть с подарками” с подсказкой.
