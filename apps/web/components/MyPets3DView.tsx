...
function SceneLoadingOverlay() {
  ...
  const tips = [
    ...
    "Переключайтесь между мемориалами стрелками или касанием сцены."
  ];
...
export default function MyPets3DView({
  ...
  const containerClassName = fullScreen
    ? `fixed inset-0 z-0 h-[100dvh] max-h-[100dvh] w-screen overflow-hidden ${hudCanvasBackgroundClass} overscroll-none`
    : `relative h-[calc(100vh-220px)] min-h-[520px] w-full overflow-hidden rounded-[28px] border border-[#eadfd9] ${hudCanvasBackgroundClass} shadow-sm`;
  ...
  const infoAsideClass = isPortraitLayout
    ? `absolute bottom-[calc(5dvh+env(safe-area-inset-bottom))] left-2 right-2 z-30 h-[min(45dvh,25rem)] overflow-hidden overscroll-contain ${hudInfoPanelChromeClass(true)}`
    : `absolute right-6 top-1/2 z-20 flex h-[min(70dvh,44rem)] max-h-[calc(100dvh-var(--app-header-height,56px)-1.5rem)] w-[min(420px,calc(100vw-2rem))] -translate-y-1/2 flex-col overflow-hidden ${hudInfoPanelChromeClass(false)} [@media(max-height:640px)]:right-4 [@media(max-height:640px)]:h-[calc(100dvh-var(--app-header-height,56px)-1rem)] [@media(max-height:640px)]:rounded-[24px] [@media(max-height:640px)]:p-2`;
  const infoCardClass = isPortraitLayout
    ? `relative flex h-full min-h-0 flex-col overflow-y-auto p-3 overscroll-contain ${hudCardSurfaceClass(true)}`
    : `relative flex h-full min-h-0 flex-col overflow-y-auto p-4 ${hudCardSurfaceClass(false)} [@media(max-height:640px)]:rounded-[20px] [@media(max-height:640px)]:p-3`;
  ...
  const infoImageFallbackClass = isPortraitLayout
    ...
    : "mx-auto h-[clamp(7rem,24dvh,12rem)] w-[clamp(7rem,24dvh,12rem)] rounded-[28px] bg-[#eadfd9] [@media(max-height:640px)]:h-[clamp(5.5rem,22dvh,8rem)] [@media(max-height:640px)]:w-[clamp(5.5rem,22dvh,8rem)] [@media(max-height:640px)]:rounded-[22px]";
  ...
  return (
    <div className={containerClassName}>
      ...
      {pets.length === 0 && !loading ? (
        <div className="absolute inset-0 z-10 flex items-center justify-center text-sm font-semibold text-[#8d6e63]">
          Пока нет мемориалов.
        </div>
      ) : null}
          ...
          <div className={infoCardClass}>
            ...
            <div className={isPortraitLayout ? "mt-3 h-16 shrink-0 overflow-y-auto rounded-[16px] bg-[#f7f1ee]/80 px-3 py-2" : "mt-3 h-[clamp(4rem,15dvh,7rem)] shrink-0 overflow-y-auto rounded-[20px] bg-[#f7f1ee]/80 px-4 py-3 [@media(max-height:640px)]:mt-2 [@media(max-height:640px)]:rounded-[16px] [@media(max-height:640px)]:px-3 [@media(max-height:640px)]:py-2"}>
              <p className={isPortraitLayout ? "text-sm italic leading-snug text-[#6f6360]" : "text-[15px] italic leading-relaxed text-[#6f6360]"}>
                &ldquo;{selectedItem.pet.epitaph ?? "Теплые слова пока не указаны"}&rdquo;
              </p>
            </div>
